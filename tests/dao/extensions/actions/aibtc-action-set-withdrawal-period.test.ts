...
const contractAddress = `${deployer}.${ContractActionType.DAO_ACTION_SET_WITHDRAWAL_PERIOD}`;

describe(`action extension: ${ContractActionType.DAO_ACTION_SET_WITHDRAWAL_PERIOD}`, () => {
  ...
  it("run() fails if called directly", () => {
    const withdrawalPeriod = Cl.uint(1000);
    const receipt = simnet.callPublicFn(
      contractAddress,
      "run",
      [Cl.buffer(Cl.serialize(withdrawalPeriod))],
      deployer
    );
